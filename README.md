...
``` r
ggplot(data, aes(x = factor(is_repeated_guest), y = is_canceled, fill = factor(is_repeated_guest))) +
...
data %>%
  ...
  ggplot(aes(x = market_segment, y = cancel_rate, fill = market_segment)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = percent(cancel_rate, accuracy = 0.1)),
            vjust = -0.5, size = 4) +
  scale_y_continuous(labels = percent_format()) +
...
data %>%
  ...
  geom_bar(stat = "identity", fill="#1d3370") +
...
country_counts <- data %>%
  ...
  mutate(country = factor(country, levels = country)) #this is what allowed me to do the gradient in order, had to make it a factor
